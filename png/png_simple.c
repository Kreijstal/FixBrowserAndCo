...
int encode_png(const uint32_t *pixels, int stride, int width, int height, unsigned char **dest_out, int *dest_len_out)
{
   ...
   uint32_t crc;
   uint32_t s1, s2;
   int i, j, r, g, b, a, c, data_len, comp_len, dest_len, retval=0;

   data_len = (width*4+1)*height;
   ...
   if (!zcompress0_memory(data, data_len, &comp, &comp_len)) goto error;
...
int main(int argc, char **argv)
{
   ...
   for (i=0; i<256; i++) {
      for (j=0; j<256; j++) {
         pixels[(i+0)*width+(j+0)] = 0xFF000000 | (i << 16) | (j << 8);
         pixels[(i+256)*width+(j+0)] = 0xFF000000 | (i << 16) | (j << 0);
         pixels[(i+0)*width+(j+256)] = 0x7F000000 | ((i/2) << 16) | ((j/2) << 8);
         pixels[(i+256)*width+(j+256)] = 0x7F000000 | ((i/2) << 16) | ((j/2) << 0);
      }
   }
   ...
   if (!write_file(argv[1], odata, olen)) {
      free(odata);
